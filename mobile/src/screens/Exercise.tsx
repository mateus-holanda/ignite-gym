...
import { Box, HStack, Heading, Icon, Image, ScrollView, Text, useToast, VStack } from 'native-base';
...
export function Exercise() {
  ...
  async function fetchExerciseInfo() {
    try {
      ...
      const title = isAppError ? error.message : 'There was an error loading the exercise info.';
  ...
  async function handleExerciseRegister() {
    try {
      ...
      toast.show({
        title: 'Exercise registered to your history.',
        placement: 'top',
        bgColor: 'green.700'
      });
      ...
      const isAppError = error instanceof AppError;
      const title = isAppError ? error.message : "It wasn't possible to mark the exercise as done.";
